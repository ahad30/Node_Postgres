...
const ItemList = () => {
  ...
  const handleUpdate = (e) => {
    e.preventDefault();
    axios.put(`http://localhost:5000/api/items/${editingItemId.id}`, { name, quantity, price })
      .then(res => {
        setItems(items.map(item => item.id === editingItemId.id ? res.data : item));
  ...
  return (
    <div className="p-4">
            ...
            <tr key={item.id} className='text-center'>
              ...
              <td className="py-2 px-4 border-b">${item.price}</td>
               <td className="py-2 px-4 border-b flex space-x-2 justify-center">
                <button className="bg-green-500 text-white px-2 py-1 rounded" onClick={() => handleEdit(item)}>Edit</button>
      ...
      {editingItemId &&  <form onSubmit={handleUpdate} className="p-4 bg-gray-100 rounded flex items-center space-x-4 mb-5">
      ...
      <button  className="bg-red-500 text-white px-4 py-2 rounded"  onClick={() => setEditingItemId(null)}>Cancle</button>
    </form>}
    </div>
  );
};
